...
export class SpeechAnalyzer {
   ...
   */
  private async extractFeatures(audioBuffer: AudioBuffer): Promise<SpeechFeatures> {
    ...
    // Acoustic features
    const acousticFeatures = this.extractAcousticFeatures(channelData, sampleRate);
    
    // Voice quality features
    const voiceQualityFeatures = this.extractVoiceQualityFeatures(channelData, sampleRate);
    
    // Tremor detection
    const tremorFeatures = this.detectTremor(channelData, sampleRate);
    
    // Prosodic features
    const prosodicFeatures = this.extractProsodicFeatures(channelData, sampleRate);
  ...
  private extractTemporalFeatures(audioData: Float32Array, sampleRate: number) {
    ...
    const totalSpeechTime = speechSegments.reduce((sum, seg) => sum + (seg.end - seg.start), 0);
    ...
    // Pause analysis
    const pauseDurations = pauseSegments.map(seg => (seg.end - seg.start) * 1000); // ms
    const pauseDuration = pauseDurations.length > 0 ? 
      pauseDurations.reduce((sum, dur) => sum + dur, 0) / pauseDurations.length : 0;
    const pauseFrequency = (pauseSegments.length / (audioData.length / sampleRate)) * 60; // per minute
  ...
  private extractAcousticFeatures(audioData: Float32Array, sampleRate: number) {
    ...
    const fundamentalFreq = validF0.length > 0 ? 
      validF0.reduce((sum, f0) => sum + f0, 0) / validF0.length : 0;
    
    const f0Variability = validF0.length > 1 ? 
      Math.sqrt(validF0.reduce((sum, f0) => sum + Math.pow(f0 - fundamentalFreq, 2), 0) / (validF0.length - 1)) : 0;
   ...
   */
  private extractVoiceQualityFeatures(audioData: Float32Array, sampleRate: number) {
  ...
  private extractProsodicFeatures(audioData: Float32Array, sampleRate: number) {
    ...
    const intonationRange = validF0.length > 0 ? 12 * Math.log2(maxF0 / minF0) : 0;
    
    // Stress pattern (simplified)
    const stressPattern = this.analyzeStressPattern(audioData, sampleRate);
    
    // Rhythm variability
    const rhythmVariability = this.calculateRhythmVariability(audioData, sampleRate);
  ...
  private calculateRiskScore(features: SpeechFeatures): number {
    ...
    const speechRateRisk = features.speechRate < 120 || features.speechRate > 220 ? 
      Math.abs(features.speechRate - 175) / 175 : 0;
    riskScore += speechRateRisk * 15;
    weightSum += 15;
    
    // Pause frequency (normal: 2-4 per minute)
    const pauseRisk = features.pauseFrequency > 6 ? 
      (features.pauseFrequency - 4) / 10 : 0;
    ...
    // Jitter (normal: <1%)
    const jitterRisk = features.jitter > 0.01 ? 
      Math.min((features.jitter - 0.01) / 0.02, 1) : 0;
    riskScore += jitterRisk * 15;
    weightSum += 15;
    
    // HNR (normal: >20 dB)
    const hnrRisk = features.harmonicNoiseRatio < 15 ? 
      (20 - features.harmonicNoiseRatio) / 20 : 0;
   ...
   */
  private generateFindings(features: SpeechFeatures, riskScore: number): string[] {
  ...
  private calculateConfidence(features: SpeechFeatures): number {
    ...
    if (features.fundamentalFreq < 80 || features.fundamentalFreq > 400) confidence -= 15;
  ...
  private calculateShimmer(audioData: Float32Array, sampleRate: number): number {
    ...
    for (let i = 0; i < audioData.length - frameSize; i += hopSize) {
      const frame = audioData.slice(i, i + frameSize);
      const amplitude = Math.sqrt(frame.reduce((sum, sample) => sum + sample * sample, 0) / frame.length);
  ...
  private calculateHNR(audioData: Float32Array, sampleRate: number): number {
    ...
    const signalPower = frame.reduce((sum, sample) => sum + sample * sample, 0) / frame.length;
  ...
  private extractAmplitudeEnvelope(audioData: Float32Array, sampleRate: number): Float32Array {
    // Simplified envelope extraction
    const windowSize = Math.floor(sampleRate * 0.010); // 10ms windows
    const envelope = new Float32Array(Math.floor(audioData.length / windowSize));
  ...
  private analyzeStressPattern(audioData: Float32Array, sampleRate: number): number[] {
    // Simplified stress pattern analysis
    return [0.5, 0.3, 0.7, 0.4]; // Placeholder
  }

  private calculateRhythmVariability(audioData: Float32Array, sampleRate: number): number {
