...
export default function MultiModalAssessment({
  ...
  const runMultiModalAssessment = async () => {
    ...
    try {
      ...
      setModalities((prev) =>
        prev.map((m) => ({ ...m, status: "pending" as const })),
      );
      ...
      for (let i = 0; i < modalities.length; i++) {
        ...
        const processingTime = await simulateModalityProcessing(
          currentModality.id,
        );
  ...
  const simulateModalityProcessing = async (
    modalityId: string,
  ): Promise<number> => {
    ...
    const baseTime =
      processingTimes[modalityId as keyof typeof processingTimes] || 50;
    const actualTime = baseTime + (Math.random() - 0.5) * 10; // Add some variance

    await new Promise((resolve) =>
      setTimeout(resolve, Math.max(1000, actualTime * 10)),
    ); // Scale for demo
    return actualTime;
  };
  ...
  const calculateNRIFusion = async () => {
    ...
    const avgRiskScore =
      completedModalities.reduce(
        (sum, m) => sum + (m.result?.risk_score || 0),
        0,
            ...
            <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-6">
              ...
              <div className="grid grid-cols-1 gap-6 md:grid-cols-4">
                <div className="text-center">
                  ...
                  >
                    {nriResult.risk_category.replace("_", " ").toUpperCase()}{" "}
                    RISK
                  </div>
                </div>
                ...
                <div className="flex items-start space-x-3 rounded-lg bg-blue-500/15 border border-blue-500/30 p-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-zinc-400">
                      ...
                      {(nriResult.consistency_score * 100).toFixed(0)}%
