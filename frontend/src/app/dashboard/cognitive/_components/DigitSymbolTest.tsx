...
export default function DigitSymbolTest({
  ...
  const [responses, setResponses] = useState<{ correct: boolean; rt: number }[]>([]);
  ...
  const startTest = () => {
    logEvent("test_start", { duration_seconds: duration, items_count: ITEMS_COUNT });
    setPhase("running");
      ...
      <div className="flex flex-col items-center justify-center h-full space-y-6">
        ...
        <div className="bg-zinc-800/50 p-6 rounded-lg border border-zinc-700 max-w-lg text-center space-y-4">
          <p className="text-zinc-300">
            Match each <b>symbol</b> to its corresponding <b>number</b> using the key below.
          </p>
      ...
      >
        ...
        <div className="grid grid-cols-3 gap-4 text-center">
          <div className="bg-zinc-800 p-4 rounded-lg">
            <div className="text-3xl font-bold text-zinc-100">{currentIndex + 1}</div>
            <div className="text-xs text-zinc-500">Items Completed</div>
          </div>
          <div className="bg-zinc-800 p-4 rounded-lg">
            <div className="text-3xl font-bold text-emerald-400">{accuracy.toFixed(0)}%</div>
            <div className="text-xs text-zinc-500">Accuracy</div>
          </div>
          <div className="bg-zinc-800 p-4 rounded-lg">
            <div className="text-3xl font-bold text-zinc-100">{(avgRt / 1000).toFixed(2)}s</div>
  ...
  return (
    <div className="flex flex-col items-center h-full pt-4">
      ...
      <div className="absolute top-4 left-4 right-4 flex justify-between items-center">
        <div className={`text-lg font-mono font-bold ${timeRemaining <= 10 ? "text-red-400" : "text-zinc-300"}`}>
