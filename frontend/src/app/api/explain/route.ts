...
export async function POST(request: NextRequest) {
  try {
    ...
    const { pipeline, results, patient_context, voice_output, voice_provider } =
      body;
    ...
    const explanation =
      data.explanation || generateMockExplanation(pipeline, results);
...
function generateMockExplanation(
  ...
  if (riskScore < 30) {
    riskText = "low risk";
    recommendation =
      "Continue with routine monitoring and maintain your healthy lifestyle.";
  } else if (riskScore < 60) {
    riskText = "moderate risk";
    recommendation =
      "Consider scheduling a follow-up assessment with your healthcare provider.";
  } else {
