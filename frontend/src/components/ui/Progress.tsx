...
export const Progress: React.FC<ProgressProps> = ({
  ...
  const progressContainerClasses = cn(...containerClasses, sizeClasses[size], className);
  ...
  return (
    <div className={cn('space-y-4', className)} data-testid={testId}>
      ...
      <div className='relative'>
        ...
        <Progress value={progressPercentage} variant='clinical' size='md' animated={true} />
      </div>
              ...
              <div key={step.id} className='flex flex-col items-center space-y-2'>
                ...
                <div
                  className={cn(
                    ...
                    !isCompleted && !isCurrent && 'text-text-muted bg-neutral-700',
                  )}
                >
                  {isCompleted ? (
  ...
  return (
    <div className={cn('space-y-4', className)}>
      ...
      <div className='space-y-2 text-center'>
        <div className='text-6xl font-black text-primary-500'>{Math.round(score)}</div>
        <div className='text-text-muted text-lg'>/ 100 NRI Score</div>
        {confidence && <div className='text-text-muted text-sm'>Confidence: ±{confidence}%</div>}
      </div>

      {/* Risk Level Progress */}
      <div className='space-y-2'>
        <div className='flex items-center justify-between'>
          <span className='text-sm font-medium text-text-primary'>{riskLabels[riskLevel]}</span>
          <span className='text-sm text-text-secondary'>{Math.round(score)}%</span>
        </div>

        <Progress value={score} variant='risk' size='lg' animated={animated} />

        {showDetails && (
          <p className='text-center text-sm text-text-secondary'>{riskDescriptions[riskLevel]}</p>
        )}
      </div>
      ...
      {showDetails && (
        <div className='grid grid-cols-4 gap-2 text-xs'>
          <div className='text-center'>
            <div className='bg-success mb-1 h-2 w-full rounded' />
            <span className='text-text-muted'>0-25</span>
          </div>
          <div className='text-center'>
            <div className='bg-warning mb-1 h-2 w-full rounded' />
          ...
          <div className='text-center'>
            <div className='bg-error mb-1 h-2 w-full rounded' />
