...
const PipelineStatusContext = createContext<PipelineStatusContextType | null>(
  null,
);
...
const initialPipelineState = (): Record<PipelineType, PipelineState> => ({
  ...
  dermatology: { pipeline: "dermatology", status: "idle" },
...
export function PipelineStatusProvider({ children }: { children: ReactNode }) {
  const [pipelines, setPipelines] =
    useState<Record<PipelineType, PipelineState>>(initialPipelineState);

  const updatePipeline = useCallback(
    (pipeline: PipelineType, state: Partial<PipelineState>) => {
      setPipelines((prev) => ({
        ...prev,
  ...
  const startPipeline = useCallback(
    (pipeline: PipelineType, stageNames?: string[]) => {
    ...
    (
      ...
      setPipelines((prev) => {
        ...
        const stages = current.stages.map((s, i) => {
          ...
          if (
            status === "complete" &&
            current.stages![i - 1]?.name === stageName &&
            s.status === "pending"
          ) {
        ...
        return {
          ...
          [pipeline]: {
            ...
            stages,
            currentStage:
              stages.find((s) => s.status === "active")?.name || stageName,
            progress,
          },
        };
      });
  ...
  const completePipeline = useCallback(
    (pipeline: PipelineType, success: boolean, message?: string) => {
      setPipelines((prev) => ({
        ...prev,
        [pipeline]: {
          ...prev[pipeline],
...
export function PipelineStatusBar() {
  ...
  const activePipelines = Object.values(pipelines).filter(
    (p) => p.status !== "idle",
  );
  ...
  return (
    ...
    >
      ...
      {state.message && state.status !== "processing" && (
        <span
          className={`truncate max-w-[100px] sm:max-w-[150px] ${state.status === "error" ? "text-red-400" : "text-green-400"}`}
        >
          {state.message}
        </span>
      )}
