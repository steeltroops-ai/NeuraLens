    ...
    <div className={`rounded-xl bg-white p-6 shadow-lg ${className}`}>
      <div className='mb-6 flex items-center justify-between'>
        <div>
          ...
          <p className='mt-1 text-gray-600'>Test all frontend API endpoints</p>
        </div>
        <button
          onClick={runTests}
          disabled={isRunning}
      ...
      {isRunning && (
        ...
        >
          <div className='flex items-center gap-3'>
            ...
            <span className='font-medium text-blue-700'>{currentTest}</span>
          </div>
        </motion.div>
      )}
      ...
      {results && (
        ...
        >
          ...
          <div className={`rounded-lg border p-4 ${getStatusColor(results.overall.success)}`}>
            <div className='flex items-center justify-between'>
              ...
              <div className='flex items-center gap-2 text-sm'>
                ...
                <span>
                  {Math.round(results.overall.totalTime / results.overall.totalTests)}ms avg
                </span>
              </div>
            </div>
          </div>
            ...
            {Object.entries(results.results).map(([endpoint, result]) => (
              ...
              >
                ...
                <div className='text-right'>
                  <div className='text-sm font-medium text-gray-900'>{result.responseTime}ms</div>
                  {result.error && (
                    <div className='max-w-xs truncate text-xs text-red-600'>{result.error}</div>
                  )}
                </div>
              </motion.div>
            ))}
          ...
          {/* Test Report */}
          <div className='mt-6'>
            <h4 className='mb-3 font-semibold text-gray-900'>Test Report</h4>
          ...
          <div className='grid grid-cols-2 gap-4 md:grid-cols-4'>
            <div className='rounded-lg bg-blue-50 p-3 text-center'>
              <div className='text-2xl font-bold text-blue-600'>{results.overall.totalTests}</div>
