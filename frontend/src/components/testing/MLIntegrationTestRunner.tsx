    ...
    <div className={`bg-white rounded-xl shadow-lg p-6 ${className}`}>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-600">Test complete ML integration layer with backend API communication</p>
        </div>
        <button
          onClick={runTests}
          disabled={isRunning}
      ...
      {results && (
        ...
        >
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-lg">
                    {results.overall.success ? 'All ML Integration Tests Passed!' : 'Some ML Tests Failed'}
                  </h3>
                  <p className="text-sm opacity-75">
                    {results.overall.passedTests}/{results.overall.totalTests} tests passed in {results.overall.totalDuration}ms
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-2 text-sm">
                <Zap className="w-4 h-4" />
                <span>{Math.round(results.overall.totalDuration / results.overall.totalTests)}ms avg</span>
              </div>
            </div>
          </div>
          ...
          {/* Test Report */}
          <div className="mt-6">
            <h4 className="font-semibold text-gray-900 mb-3">ML Integration Test Report</h4>
          ...
          <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
            <h4 className="font-semibold text-blue-900 mb-2">ML Integration Status</h4>
      ...
      {!results && !isRunning && (
        <div className="text-center py-8 text-gray-500">
          ...
          <p className="text-sm mt-2">
            This will test session management, API communication, progress tracking, and error handling.
          </p>
        </div>
      )}
    </div>
