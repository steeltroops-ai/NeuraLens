...
export const SpeechAnalysisCard: React.FC<SpeechAnalysisCardProps> = ({
  ...
  useEffect(() => {
    const initializeComponents = async () => {
      try {
        ...
        const errorMessage = error instanceof SpeechAnalysisError 
          ? error.message 
        ...
        if (onError) {
          onError(error instanceof SpeechAnalysisError 
            ? error 
  ...
  const handleStartRecording = useCallback(async () => {
    try {
      ...
      const analysisError = error instanceof SpeechAnalysisError 
        ? error 
  ...
  const renderRecordingButton = () => {
    ...
    return (
      <motion.button
        ...
        className="flex h-16 w-16 items-center justify-center rounded-full bg-gradient-to-r from-blue-600 to-teal-600 text-white shadow-lg transition-all duration-200 hover:shadow-xl disabled:opacity-50 disabled:cursor-not-allowed"
    ...
    return (
      <div className="w-full">
        <div className="flex justify-between text-sm text-gray-600 mb-2">
          ...
          <span>
            {Math.round(recordingState.recordedDuration)}s / {SPEECH_ANALYSIS_CONSTANTS.DEFAULT_DURATION}s
          </span>
        </div>
        <div className="w-full bg-gray-200 rounded-full h-2">
          <div 
            className="bg-gradient-to-r from-blue-600 to-teal-600 h-2 rounded-full transition-all duration-200"
  ...
  const renderResults = () => {
    ...
    return (
      <motion.div
        ...
        className="mt-6 p-4 rounded-xl border border-gray-200 bg-white/80 backdrop-blur-sm"
      >
  ...
  if (initializationError) {
    return (
      <div className={`rounded-2xl border border-red-200 bg-red-50/80 p-6 backdrop-blur-sm ${className}`}>
  ...
  return (
    <div className={`rounded-2xl border border-gray-200 bg-white/85 p-6 shadow-lg backdrop-blur-md ${className}`}>
      {/* Header */}
      <div className="mb-6">
