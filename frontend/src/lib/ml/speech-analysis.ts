...
export class SpeechAnalyzer {
  ...
  private extractAcousticFeatures(audioData: Float32Array, sampleRate: number) {
    ...
    const fundamentalFreq =
      validF0.length > 0 ? validF0.reduce((sum, f0) => sum + f0, 0) / validF0.length : 0;

    const f0Variability =
      validF0.length > 1
        ? Math.sqrt(
            validF0.reduce((sum, f0) => sum + Math.pow(f0 - fundamentalFreq, 2), 0) /
              (validF0.length - 1),
          )
        : 0;
  ...
  private calculateRiskScore(features: SpeechFeatures): number {
    ...
    const pauseRisk = features.pauseFrequency > 6 ? (features.pauseFrequency - 4) / 10 : 0;
    ...
    // Jitter (normal: <1%)
    const jitterRisk = features.jitter > 0.01 ? Math.min((features.jitter - 0.01) / 0.02, 1) : 0;
    riskScore += jitterRisk * 15;
    weightSum += 15;

    // HNR (normal: >20 dB)
    const hnrRisk = features.harmonicNoiseRatio < 15 ? (20 - features.harmonicNoiseRatio) / 20 : 0;
